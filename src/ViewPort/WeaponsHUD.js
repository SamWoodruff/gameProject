...
const WeaponsHUD = ({ currentScore, selectedWeapon, updateEquippedWeapon }) => {
  const keyPress = e => {
    if (e.keyCode === 49)
      if (currentScore >= blasters[0].points) updateEquippedWeapon(blasters[0]);
    if (e.keyCode === 50)
      if (currentScore >= blasters[1].points) updateEquippedWeapon(blasters[1]);
    if (e.keyCode === 51)
      if (currentScore >= blasters[2].points) updateEquippedWeapon(blasters[2]);
  };
